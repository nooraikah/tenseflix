...
function loadGallery(userProgress) {
    ...
    DISPLAY_ORDER.forEach((item) => {
        ...
        galleryHTML += `
            <div class="gallery-item" style="background:#0f0f1e; border-radius:15px; overflow:hidden; border: 3px solid ${isUnlocked ? '#e2b714' : '#1a1a2e'}; opacity: ${isUnlocked ? '1' : '0.25'}; transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); cursor: pointer; box-shadow: 0 10px 30px rgba(0,0,0,0.5);" onclick="${isUnlocked ? `openMovieLightbox('${item.id}')` : ''}">
                <img src="${isUnlocked ? item.id + '.png' : 'placeholder.png'}" style="width:100%; height:240px; object-fit:cover; background:#000; display:block; transition: transform 0.5s ease;">
