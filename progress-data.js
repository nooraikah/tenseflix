...
function restoreFromSavedState() {
    ...
    // ── Restore Filters (Difficulty/Form) ────────────────────────────────────
    if (state.currentDifficulty && typeof setDifficulty === 'function') {
        setDifficulty(state.currentDifficulty);
    }
    if (state.currentSentenceForm && typeof setSentenceForm === 'function') {
        setSentenceForm(state.currentSentenceForm);
    }
...
function saveVideoTime(tenseId, videoNumber, currentTime) {
    // Feature disabled: Videos always start from 0:00.
}
